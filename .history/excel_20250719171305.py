...
def create_cost_sheet_template():
    ...
    for cell, value, merged_range, font_name, font_size, bold, italic, underline, font_color, bg_color, border_left, border_top, border_right, border_bottom in template_data:
        ...
        border_sides = {}
        if border_left != "none":
            border_sides['left'] = Side(style=border_left)
        if border_top != "none":
            border_sides['top'] = Side(style=border_top)
        if border_right != "none":
            border_sides['right'] = Side(style=border_right)
        if border_bottom != "none":
            border_sides['bottom'] = Side(style=border_bottom)
...
def main(minio_access_key, minio_secret_key, bucket_name, dynamic_data=None):
    ...
    # 2. เพิ่มข้อมูลแบบ dynamic (ถ้ามี)
    if dynamic_data:
