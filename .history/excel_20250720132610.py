...
def insert_dynamic_data(wb, columns, rows):
    ...
    # เพิ่ม outside borders สำหรับแต่ละ type
    thin = Side(border_style="thin", color="000000")
    ...
    for type_code in type_order:
        if type_code in type_start_rows:
            ...
            # ใส่ outside borders - เฉพาะเส้นขอบนอก
            for row in range(start_row, end_row + 1):
                # Top row ของ type
                if row == start_row:
                    # Column A - top, left
                    ws[f'A{row}'].border = Border(top=thin, left=thin)
                    # Column B-K - top
                    for col in range(2, 12):  # B to K
                        cell = ws.cell(row=row, column=col)
                        cell.border = Border(top=thin)
                    # Column K - right
                    ws[f'K{row}'].border = Border(top=thin, right=thin)
                
                # Bottom row ของ type
                elif row == end_row:
                    # Column A - left, bottom
                    ws[f'A{row}'].border = Border(left=thin, bottom=thin)
                    # Column B-K - bottom
                    for col in range(2, 12):  # B to K
                        cell = ws.cell(row=row, column=col)
                        cell.border = Border(bottom=thin)
                    # Column K - right, bottom
                    ws[f'K{row}'].border = Border(right=thin, bottom=thin)
                
                # Middle rows
                else:
                    # Column A - left only
                    ws[f'A{row}'].border = Border(left=thin)
                    # Column K - right only
                    ws[f'K{row}'].border = Border(right=thin)
