...
def insert_dynamic_data(wb, data, template_type="cost_sheet"):
    """เพิ่มข้อมูลแบบ dynamic ลงใน template"""
    ws = wb.active
    
    if template_type == "cost_sheet":
        # สำหรับ cost sheet template - ใส่ข้อมูลในตำแหน่งที่กำหนด
        for cell_position, value in data.items():
            if cell_position in ws:
                ws[cell_position] = value
    else:
        # สำหรับ template แบบเดิม - ใส่ข้อมูลในตำแหน่งที่กำหนด
        for cell_position, value in data.items():
            if cell_position in ws:
                ws[cell_position] = value
...
def insert_item_details(wb, columns, rows):
    """ใส่รายการข้อมูลตาม Type ที่ match กับ template"""
    ws = wb.active
    ...
    # กำหนดตำแหน่งเริ่มต้นของแต่ละ Type ใน template
    type_start_rows = {
        'stucture': 13,  # เริ่มจาก A13
        'Graphic': 16,   # เริ่มจาก A16
        'Furniture&plant': 19,  # เริ่มจาก A19
        'Electtrical': 22  # เริ่มจาก A22
    }
    ...
    for template_type, items in type_groups.items():
        if template_type in type_start_rows:
            start_row = type_start_rows[template_type]
...
def main(columns, rows, minio_access_key, minio_secret_key, bucket_name, template_type="cost_sheet", dynamic_data=None):
    ...
    if template_type == "cost_sheet":
        ...
        # 1. สร้าง Excel cost sheet template
        wb = create_cost_sheet_template()
        
        # 2. ใส่ข้อมูลจาก columns และ rows
        insert_cost_sheet_data(wb, columns, rows)
        
        # 3. ใส่ข้อมูลแบบ dynamic (ถ้ามี)
        if dynamic_data:
            insert_dynamic_data(wb, dynamic_data, template_type)
    else:
        file_name = f"NCC-{now}.xlsx"
        # 1. สร้าง Excel แบบเดิม
        wb = openpyxl.Workbook()
        ws = wb.active
        if ws is None:
            raise ValueError("Failed to create worksheet")
        ...
        # ใส่ข้อมูลแบบ dynamic (ถ้ามี)
        if dynamic_data:
            insert_dynamic_data(wb, dynamic_data, template_type)

    object_key = f"excel-sheet/{file_name}"

    # 4. บันทึกไฟล์ใน memory
    ...
    # 7. สร้าง Presigned URL แบบ public ให้คนอื่นโหลดได้
...
if __name__ == "__main__":
    ...
    dynamic_data = {
        "A6": "John Doe",  # Sales
        "A7": "Jane Smith",  # Designer
        "D4": "500,000",  # Budget
        "D6": "Design Team",  # Estimate By
        "D7": "Final version"  # Note
    }
    
    # เรียกใช้ฟังก์ชัน (ใส่ credentials จริง)
    # result = main(columns, rows, "your_access_key", "your_secret_key", "your_bucket", "cost_sheet", dynamic_data)
    # print(result)
