...
def insert_dynamic_data(wb, columns, rows):
    ...
    for type_code in type_order:
        if type_code in type_start_rows:
            ...
            end_row = current_row - 1  # row สุดท้ายของ type นี้ (TOTAL row)
            ...
            for row in range(start_row, end_row + 1):
                # Top border สำหรับ row แรก
                if row == start_row:
                    ws[f'A{row}'].border = Border(top=thick, left=thick, right=thin, bottom=thin)
                    for col in range(2, 12):  # B to K
                        cell = ws.cell(row=row, column=col)
                        cell.border = Border(top=thick, left=thin, right=thin, bottom=thin)
                
                # Bottom border สำหรับ row สุดท้าย
                elif row == end_row:
                    ws[f'A{row}'].border = Border(top=thin, left=thick, right=thin, bottom=thick)
                    for col in range(2, 12):  # B to K
                        cell = ws.cell(row=row, column=col)
                ...
                # Middle rows
                    ...
                    ws[f'A{row}'].border = Border(top=thin, left=thick, right=thin, bottom=thin)
                    for col in range(2, 12):  # B to K
                        cell = ws.cell(row=row, column=col)
