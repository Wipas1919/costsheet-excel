...
def insert_dynamic_data(wb, columns, rows):
    ...
    for type_code in type_order:
        ...
        for component_code in component_order:
            ...
            # ใส่ข้อมูลรายการใน row เดียวกับ component group
            if data_by_component[component_code]:
                first_row_data = data_by_component[component_code][0]
                if isinstance(first_row_data, list):
                    for col_idx, col_name in enumerate(columns):
                        if col_idx < len(first_row_data):
                            cell_value = first_row_data[col_idx]
                            cell = ws.cell(row=current_row, column=col_idx + 3)  # เริ่มจาก column C (index 3)
